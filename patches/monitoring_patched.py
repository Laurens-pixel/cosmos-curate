...
def get_ray_actors(state: str = "ALIVE") -> List[ActorInfo]:
    ...
    # Patch: pass the current Ray instance's GCS address explicitly to avoid
    # ConnectionError when multiple Ray instances are running (e.g. in Apptainer)
    try:
        gcs_address = ray.get_runtime_context().gcs_address
    except Exception:
        gcs_address = None
    try:
        actors_data: Any = ray.util.state.list_actors(
            address=gcs_address, filters=[("state", "=", "ALIVE")], limit=limit
        )
    except ConnectionError:
        logger.warning("Could not list Ray actors (multiple instances detected). Returning empty list.")
        return []
    actor_infos = []
